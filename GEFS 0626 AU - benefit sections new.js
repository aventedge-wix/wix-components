...
// FIX 1: Google Fonts injected as a <link> node appended to the shadow root.
// @import inside a Shadow DOM <style> tag is unreliable and was causing the
// element to render blank in Wix Studio.
...
const CSS = `
  ...
  *, *::before, *::after {
  ...
  /* FIX 2: backdrop-filter removed — blocked by Wix's CSP in most contexts
     and can silently prevent the entire element from painting. Replaced with
     a plain semi-transparent white background on hover. */
  .benefit-item:hover {
  ...
  .icon-wrapper {
    ...
    transition: transform 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275),
                background 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275),
...
class BenefitSections extends HTMLElement {
  connectedCallback() {
    ...
    // FIX 1: <link> injected into shadow root — the only reliable way to load
    // Google Fonts inside Shadow DOM in Wix Studio.
    ...
    BENEFITS.forEach(({ title, description, icon }) => {
      ...
      // Parse icon SVG string, transplant inner paths into a namespaced SVG
      // so stroke/fill attributes survive Shadow DOM encapsulation correctly.
      ...
      svg.classList.add("benefit-icon");
    ...
    // Order matters: fontLink first so font is requested before text paints
